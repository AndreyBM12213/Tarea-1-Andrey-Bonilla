...
public class GiftListRestController {
    ...
    public ResponseEntity<?> addGiftToGiftList(@PathVariable Long giftListId, @RequestBody Gift gift, HttpServletRequest request) {
        ...
        if(foundGiftList.isPresent()) {
            ...
            return new GlobalResponseHandler().handleResponse("Product added to Category successfully",
                    savedGift, HttpStatus.CREATED, request);
        } else {
            return new GlobalResponseHandler().handleResponse("Category " + giftListId + " not found",
                    HttpStatus.NOT_FOUND, request);
        }
    }
    ...
    public ResponseEntity<?> getGiftsFromGiftList(
        ...
        if(foundGiftList.isPresent()) {
            ...
            return new GlobalResponseHandler().handleResponse("Product from Category retrieved successfully",
    ...
    public ResponseEntity<?> removeGiftFromGiftList(@PathVariable Long giftListId, @PathVariable Long giftId, HttpServletRequest request) {
        ...
        if(foundGiftList.isPresent() && foundGift.isPresent()) {
            ...
            if(gift.getGiftList().getId().equals(giftListId)) {
                ...
                return new GlobalResponseHandler().handleResponse("Product removed from Category successfully",
                        gift, HttpStatus.OK, request);
            } else {
                return new GlobalResponseHandler().handleResponse("Product " + giftId + " does not belong to Category " + giftListId,
                        HttpStatus.BAD_REQUEST, request);
            }
        } else {
            return new GlobalResponseHandler().handleResponse("Category or Product not found",
                    HttpStatus.NOT_FOUND, request);
        }
    }
